 ...
 */
export const processarDREDiaria = async (restaurantId: string, referenceDate: Date) => {
  ...
  const [vendas] = await db
    .select({
      ...
      custoEstimado: sql<number>`sum(${ordersTable.estimatedCost})`,
    })
    .from(ordersTable)
    .where(
      and(
        ...
        sql`${ordersTable.createdAt} BETWEEN ${startOfDay} AND ${endOfDay}`
      )
    );
  ...
  const [outrasReceitas] = await db
    .select({
      total: sql<number>`sum(${financialTransactionsTable.amount})`,
    })
    ...
    .where(
      and(
        ...
        sql`${financialTransactionsTable.paidAt} BETWEEN ${startOfDay} AND ${endOfDay}`
      )
    );
