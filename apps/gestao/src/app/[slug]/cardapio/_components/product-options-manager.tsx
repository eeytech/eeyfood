...
import { ChevronDownIcon, ChevronUpIcon, PencilIcon, PlusIcon, Trash2Icon, XIcon } from "lucide-react";
...
function GroupForm({
  ...
  const set = (field: keyof GroupFormState) => (e: React.ChangeEvent<HTMLInputElement>) =>
    ...
    <div className="space-y-3 rounded-xl border bg-slate-50 p-4">
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div className="space-y-1">
          ...
          <Input type="number" min="0" value={form.minOptions} onChange={set("minOptions")} className="h-9" />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">Máximo de opções</Label>
          <Input type="number" min="1" value={form.maxOptions} onChange={set("maxOptions")} className="h-9" />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">Ordem de exibição</Label>
          <Input type="number" min="0" value={form.displayOrder} onChange={set("displayOrder")} className="h-9" />
        </div>
      </div>
      <div className="flex justify-end gap-2">
        ...
        <Button type="button" size="sm" onClick={() => onSubmit(form)} disabled={isPending || !form.name.trim()}>
...
function OptionForm({
  ...
  const set = (field: keyof OptionFormState) => (e: React.ChangeEvent<HTMLInputElement>) =>
    setForm((prev) => ({ ...prev, [field]: e.target.value }));
    ...
    <div className="space-y-3 rounded-xl border bg-white p-3 shadow-sm">
      <div className="grid gap-2 sm:grid-cols-2">
        <div className="space-y-1 sm:col-span-2">
          ...
          <Input value={form.name} onChange={set("name")} placeholder="Ex: Grande, Barbecue, Bacon extra..." className="h-8 text-sm" />
        </div>
        <div className="space-y-1 sm:col-span-2">
          <Label className="text-xs">Descrição (opcional)</Label>
          <Input value={form.description} onChange={set("description")} placeholder="Descrição curta" className="h-8 text-sm" />
        </div>
        <div className="space-y-1 sm:col-span-2">
          <Label className="text-xs">URL da imagem (opcional)</Label>
          <Input value={form.imageUrl} onChange={set("imageUrl")} placeholder="https://..." className="h-8 text-sm" />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">Preço adicional (R$)</Label>
          <Input type="number" step="0.01" min="0" value={form.price} onChange={set("price")} className="h-8 text-sm" />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">Ordem</Label>
          <Input type="number" min="0" value={form.displayOrder} onChange={set("displayOrder")} className="h-8 text-sm" />
        </div>
      </div>
      <div className="flex justify-end gap-2">
        <Button type="button" variant="ghost" size="sm" onClick={onCancel} disabled={isPending}>
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg border bg-white px-3 py-2">
      ...
      <div className="flex shrink-0 gap-1">
        <Button type="button" variant="ghost" size="icon" className="h-7 w-7" onClick={() => setEditing(true)} disabled={isPending}>
          <PencilIcon size={12} />
        </Button>
        <Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-rose-500 hover:bg-rose-50" onClick={handleDelete} disabled={isPending}>
          <Trash2Icon size={12} />
        </Button>
      </div>
    </div>
    ...
    <div className="overflow-hidden rounded-xl border bg-slate-50">
      ...
      >
        ...
        <div className="flex shrink-0 gap-1" onClick={(e) => e.stopPropagation()}>
          <Button type="button" variant="ghost" size="icon" className="h-7 w-7" onClick={() => setEditingGroup(true)} disabled={isPendingGroup}>
            <PencilIcon size={12} />
          </Button>
          <Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-rose-500 hover:bg-rose-50" onClick={handleDeleteGroup} disabled={isPendingGroup}>
            <Trash2Icon size={12} />
          </Button>
        </div>
      </div>
      ...
      {expanded && (
        <div className="space-y-2 border-t bg-slate-50/50 p-3">
          {group.options.map((option) => (
            <OptionRow key={option.id} option={option} slug={slug} onRefresh={onRefresh} />
          ))}
          ...
          {addingOption ? (
            ...
            <Button type="button" variant="outline" size="sm" className="w-full gap-1.5 border-dashed" onClick={() => setAddingOption(true)}>
              <PlusIcon size={13} />
              Adicionar item
            </Button>
          )}
...
export function ProductOptionsManager({ slug, productId }: ProductOptionsManagerProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {groups.map((group) => (
        <GroupCard key={group.id} group={group} slug={slug} productId={productId} onRefresh={handleRefresh} />
      ))}
      ...
      {addingGroup ? (
        ...
        <Button type="button" variant="outline" className="w-full gap-2 border-dashed" onClick={() => setAddingGroup(true)}>
          <PlusIcon size={14} />
