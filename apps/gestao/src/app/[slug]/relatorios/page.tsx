      ...
      <Card className="overflow-hidden border-white/80 bg-white/90">
        <CardHeader>
          <div className="flex items-center justify-between gap-4">
            <div>
              <CardTitle className="font-display text-xl">Relatórios</CardTitle>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card className="border-white/80 bg-white/90">
          <CardContent className="flex items-center justify-between p-4">
            ...
            <WalletCardsIcon className="text-primary" size={20} />
          </CardContent>
        </Card>
        <Card className="border-white/80 bg-white/90">
          <CardContent className="flex items-center justify-between p-4">
        ...
        <Card className="border-white/80 bg-white/90">
          <CardContent className="flex items-center justify-between p-4">
            ...
            <BarChart3Icon className="text-amber-500" size={20} />
          </CardContent>
        </Card>
        <Card className="border-white/80 bg-white/90">
          <CardContent className="flex items-center justify-between p-4">
            ...
            <PackageSearchIcon className="text-slate-600" size={20} />
          </CardContent>
        </Card>
      </div>
                ...
                {report.history.length === 0 ? (
                  ...
                  report.history.map((row) => (
                    <tr key={row.referenceDate} className="border-b last:border-0">
                      <td className="px-2 py-2 font-medium">{formatDate(row.referenceDate)}</td>
                      <td className="px-2 py-2">{String(row.totalOrders)}</td>
                      <td className="px-2 py-2">{formatCurrency(row.grossRevenue)}</td>
          ...
          <CardContent className="space-y-3">
            {report.topProducts.length === 0 ? (
              ...
              report.topProducts.map((product, index) => (
                <div key={product.productId} className="rounded-xl border bg-slate-50/80 p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
                        #{String(index + 1)}
                      </p>
                      <h3 className="mt-0.5 text-sm font-semibold">{product.productName}</h3>
                    </div>
